...
#  end
#
#  it "always returns a quality_of of zero for the first call" do
#    @o.quality_of([1,2,3]).must_equal 0.0
#  end
#
#  it "handles a more complex series of consecutive calls" do
#    # Set first values => quality_of is always zero
#    @o.quality_of([1,2,3]).must_equal 0.0
#
#    # Now we come with a worse candidate => still zero
#    @o.quality_of([1,2,5]).must_equal 0.0
#
#    # But now the previous value is the best candidate we have seen so gets maximum quality value
#    @o.quality_of([1,2,3]).must_equal 1.0
#
#    # The previous worst is still the worst
#    @o.quality_of([1,2,5]).must_equal 0.0
#
#    # And now some complex ones that are between the prev best and worst
#    @o.quality_of([1,2,4]).must_equal (((4.0 - 3.0)/(4-2) + (8.0 - 7)/(8-6))/2)
#    @o.quality_of([1,2,4.5]).must_equal (((4.0 - 3.5)/(4-2) + (8.0 - 7.5)/(8-6))/2)
#
#    # Now extend the global best with a new best
#    @o.quality_of([1,2,2]).must_equal 1.0 # new global min = [1, 5] and max the same at [4, 8]
#
#    # And the in between candidates now have new values based on the new mins
#    @o.quality_of([1,2,4]).must_equal (((4.0 - 3.0)/(4-1) + (8.0 - 7)/(8-5))/2)
#    @o.quality_of([1,2,4.5]).must_equal (((4.0 - 3.5)/(4-1) + (8.0 - 7.5)/(8-5))/2)
#  end
#end
#
